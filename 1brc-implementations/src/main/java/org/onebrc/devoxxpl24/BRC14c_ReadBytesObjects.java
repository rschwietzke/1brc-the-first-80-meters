...
import org.onebrc.util.ParseDouble;
 ...
 */
public class BRC14c_ReadBytesObjects extends Benchmark
{
    /**
            ...
            int hash)
    {
        public City getCity()
        {
            return new City(Arrays.copyOfRange(data, lineStartPos, semicolonPos), hash);
        }
    ...
    static class Reader
    {
        ...
        private int hashCode(final int lineStartPos, final int semicolonPos)
        {
            int h = 1;
            for (int i = lineStartPos; i < semicolonPos; i++)
            {
                h = 31 * h + data[i];
            }
            ...
            for (; i < end; i++)
            {
                ...
                if (b == '\n')
                {
                    ...
                    return new Line(
                            ...
                            semicolonPos,
                            newlinePos,
                            hashCode(lineStartPos, semicolonPos));
                }
            }
    ...
    record City(byte[] data, int hash)
    {
    ...
    public String run(final String fileName) throws IOException
    {
        ...
        {
            final Reader reader = new Reader(channel);
            Line line;

            while ((line = reader.readFromChannel()) != null)
            {
    ...
    {
        Benchmark.run(BRC14c_ReadBytesObjects.class, args);
    }
