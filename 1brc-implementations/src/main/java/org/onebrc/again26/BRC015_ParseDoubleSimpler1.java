...
/**
 * We update our parseDouble to avoid some overhead. This is very
 * small tuning already, just to save some cycles.
 *
 * @author Rene Schwietzke
 */
public class BRC015_ParseDoubleSimpler1 extends Benchmark
{
    /**
    ...
    public String run(final String fileName) throws IOException
    {
    	...
    	try (var reader = Files.newBufferedReader(Paths.get(fileName)))
        {
    		...
    		while ((line = reader.readLine()) != null)
    		{
    			...
    			final double temperature = parseDouble(temperatureString);
    ...
    {
        // this new version avoids to touch the first character twice
        // because we already checked it, we can also store it as number
        // it is one, which makes the loop start at index 1 always
        ...
        int decimalPos;
        final int end = decimalPos = s.length();
        ...
        return value * multipliers[end - decimalPos];
    }

    public static void main(String[] args) throws NoSuchMethodException, SecurityException
    {
		Benchmark.run(BRC015_ParseDoubleSimpler1.class, args);
    }
}
