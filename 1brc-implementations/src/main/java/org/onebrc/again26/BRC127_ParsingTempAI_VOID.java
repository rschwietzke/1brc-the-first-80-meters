 ...
 */
public class BRC127_ParsingTempAI_VOID extends Benchmark
{
    /**
...
//        }
    ...
    public static class Line
    {
        ...
        private int parseTemperature(int totalRead)
        {
            ...
            // 1. Check for negative sign (ASCII 45)
            if (this.backingArray[totalRead] == '-') 
            {
                sign = -1;
                totalRead++;
            }
            
            // 2. Parse the first digit
            // Subtracting '0' (ASCII 48) converts the byte to its integer value
            result = this.backingArray[totalRead++] - '0';
            
            // 3. Check if the next byte is a dot (ASCII 46) or the second digit
            if (this.backingArray[totalRead] != '.') 
            {
                result = result * 10 + (this.backingArray[totalRead++] - '0');
            }
            
            // 4. Skip the dot
            totalRead++; 
            
            // 5. Parse the final decimal digit
            result = result * 10 + (this.backingArray[totalRead] - '0');
            
            // 6. Apply sign
            this.temperature = result * sign;
    ...
    {
        Benchmark.run(BRC127_ParsingTempAI_VOID.class, args);
    }
}
