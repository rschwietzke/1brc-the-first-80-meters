...
/**
 * We reduce some double parsing overhead even more by avoiding
 * some calculations.
 *
...
public class BRC017_ParseDoubleSimpler2 extends Benchmark
{
    ...
    public String run(final String fileName) throws IOException
    {
    	...
    	try (var reader = Files.newBufferedReader(Paths.get(fileName)))
        {
    		...
    		while ((line = reader.readLine()) != null)
    		{
    		    ...
    		    // due to our line format, reduce some overhead here, remove - 1
    			final double temperature = parseDouble(line, semicolon + 1, line.length());
    ...
    {
        long value = s.charAt(offset) - DIGITOFFSET;
        boolean negative; 
        if (negative = (value == ('-' - DIGITOFFSET)))
        {
            value = 0;
        }
        ...
        return negative ? -value * multipliers[end - decimalPos] : value * multipliers[end - decimalPos];
    }
