...
/**
 * Chars are overhead when reading 13 GB and turning that int
 * 26 GB of char, for mostly nothing. Numbers and control
 * chars including ; and . are ASCII. So delay unicode conversion.
 * 
...
public class BRC060_ReadingBytes extends Benchmark
{
    ...
    public String run(final String fileName) throws IOException
    {
        ...
        // open the file
        try (var stream = new BufferedInputStream(Files.newInputStream(Paths.get(fileName))))
        {
    ...
    public static class Line
    {
        ...
        public boolean readLine(final InputStream stream) throws IOException
        {
            // for the moment, this is a rather simple concept
            // which is not optimized for speed, but just top down 
            // programmed by hand... yes, there are people doing this, still
            ...
            length++; // start after semicolon
            
            // ok, same till \n
            for (; (read = stream.read()) > -1 ; length++)
            {
                byte b = (byte) read;
                // even if we don't get one, we have the last oos
                this.data[length] = b;
                if (b == '\n')
                {
                    break;
            ...
            // parse temperature
//            System.out.format("%s: %d, %d%n", c, this.semicolon, length - 1);
            this.temperature = parseInteger(this.data, this.semicolon + 1, length); // skip newline
