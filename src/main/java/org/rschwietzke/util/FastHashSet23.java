...
public class FastHashSet23
{
	// we need only the reference, not the content
    private static final City23 FREE_KEY = new City23();
    private static final City23 REMOVED_KEY = new City23();

    /** Keys and values */
    private City23[] m_data;
    ...
    public FastHashSet23()
    {
        this(13, 0.5f);
    }

    public FastHashSet23( final int size, final float fillFactor )
    {
        ...
        m_data = new City23[capacity];
        Arrays.fill( m_data, FREE_KEY );

        m_threshold = (int) (capacity * fillFactor);
    }

    public City23 getPutOnEmpty( final City23 key, LineInfo line )
    {
    ...
    public City23 put(final City23 key)
    {
        int ptr = key.hashCode() & m_mask;
        City23 k = m_data[ptr];
    ...
    private void rehash( final int newcapacity )
    {
        ...
        final City23[] oldData = m_data;

        m_data = new City23[newcapacity];
        Arrays.fill( m_data, FREE_KEY );
     ...
     * @return
     */
    public List<City23> keys()
    {
        final List<City23> result = new ArrayList<>(this.m_size);

        final int length = m_data.length;
        for (int i = 0; i < length; i++)
        {
            final City23 o = m_data[i];
